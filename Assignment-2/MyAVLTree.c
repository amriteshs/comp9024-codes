...
void PrintAVLTreeNode(AVLTreeNode *N, int r);
...
void PrintAVLTreeNode(AVLTreeNode *N, int r)
{
    ...
    PrintAVLTreeNode(N -> left, r);    // recursive call to left child of current node
    N -> rank = r;
    printf("(%d, %d), %d, %d\n", N -> key, N -> value, N -> height, N -> rank);    // print key, value, and height of current node
    PrintAVLTreeNode(N -> right, r + 1);   // recursive call to left child of current node
}
...
void PrintAVLTree(AVLTree *T)
{
    ...
    PrintAVLTreeNode(T -> root, 0);

    printf("\n");
}

int RC(AVLTreeNode *N, int lo, int hi)
{
    if (N == NULL) {
        return 0;
    }
    ...
    count += RC(N -> left, lo, hi);
    count += RC(N -> right, lo, hi);

    return count;
}

int RCR(AVLTreeNode *N, int lo, int hi)
{
    if (N == NULL) {
        return 0;
    }
    ...
    if (N -> key < lo) {
        return RCR(N -> right, lo, hi);
    }

    if (N -> key > hi) {
        return RCR(N -> left, lo, hi);
    }
...
void RangeCount(AVLTree *T, int lo, int hi)
{
...
int main() //sample main for testing
{
    ...
    j = InsertNode(tree6, 1, 1);
    j = InsertNode(tree6, 1, 2);
    j = InsertNode(tree6, 0, 0);
    j = InsertNode(tree6, 0, 1);
    j = InsertNode(tree6, 1, 3);
    j = InsertNode(tree6, 0, 2);
    j = InsertNode(tree6, 0, 3);

    PrintAVLTree(tree6);

    RangeCount(tree6, 0, 1);
